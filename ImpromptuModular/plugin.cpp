...
//
// Based on code from the Fundamental and AudibleInstruments plugins by Andrew
// Belt and graphics from the Component Library by Pyer See ./LICENSE.md for all
// licenses See ./res/fonts/ for font licenses
//***********************************************************************************************
...
static const char noteLettersSharp[12] = {'C', 'C', 'D', 'D', 'E', 'F',
                                          'F', 'G', 'G', 'A', 'A', 'B'};
static const char noteLettersFlat[12] = {'C', 'D', 'D', 'E', 'E', 'F',
                                         'G', 'G', 'A', 'A', 'B', 'B'};
static const char isBlackKey[12] = {0, 1, 0, 1, 0, 0, 1, 0, 1, 0, 1, 0};

void printNoteNoOct(int note, char *text,
                    bool sharp) { // text must be at least 3 chars long (three
                                  // displayed chars plus end of string)
  // given note is a pitch CV multiplied by 12 and rounded to nearest integer
  note = eucMod(note, 12);
  text[0] =
      sharp ? noteLettersSharp[note] : noteLettersFlat[note];     // note letter
  text[1] = (isBlackKey[note] == 1) ? (sharp ? '\"' : 'b') : ' '; // sharp/flat
  text[2] = 0;
}

int printNoteOrig(float cvVal, char *text,
                  bool sharp) { // text must be at least 4 chars long (three
                                // displayed chars plus end of string)
...
int printNote(float cvVal, char *text,
              bool sharp) { // text must be at least 4 chars long (three
                            // displayed chars plus end of string)
  // return cursor position of eos
...
void NormalizedFloat12Copy(float *float12) {
  ...
  json_object_set_new(clipboardJ, "Impromptu normalized float12",
                      normFloats12J);
  char *float12Clip =
      json_dumps(clipboardJ, JSON_INDENT(2) | JSON_REAL_PRECISION(9));
...
void NormalizedFloat12Paste(float *float12) {
  ...
  if (!float12Clip) {
    ...
    if (!clipboardJ) {
      ...
      json_t *normFloats12J =
          json_object_get(clipboardJ, "Impromptu normalized float12");
      if (!normFloats12J) {
        WARN("Error no Impromptu normalized float12 values present in "
             "clipboard");
      } else {
        json_t *normFloats12ArrayJ =
            json_object_get(normFloats12J, "normalizedFloats");
