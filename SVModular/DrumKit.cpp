...
void setupSamples() {
  ...
  // kick is 55hz
  // Sample (uint64_t _length, string _name, float *_values, float _pitch) :
  // length(_length), name(_name), values(_values), pitch(_pitch) { }
  size_t size;
  float *arr = loadSample("res/samples/kick/01.raw", &size);
  DrumKit::Sample *kick =
      new DrumKit::Sample((uint64_t)size, "kick01", arr, 55);
  ...
  for (uint8_t i = 1; i < 17; i++) {
    ...
    DrumKit::Sample *bd9 =
        new DrumKit::Sample((uint64_t)size, std::string("bd9-") + buf, arr, 0);
  ...
  for (uint8_t i = 1; i < 17; i++) {
    ...
    DrumKit::Sample *snare = new DrumKit::Sample(
        (uint64_t)size, std::string("snare-") + buf, arr, 0);
  ...
  for (uint8_t i = 1; i < 16; i++) {
    ...
    sprintf(buf, "%02d", i);
    arr =
        loadSample(std::string("res/samples/closedhh/") + buf + ".raw", &size);
    DrumKit::Sample *hh = new DrumKit::Sample(
        (uint64_t)size, std::string("closedhh-") + buf, arr, 0);
  ...
  for (uint8_t i = 1; i < 15; i++) {
    ...
    DrumKit::Sample *hh = new DrumKit::Sample(
        (uint64_t)size, std::string("openhh-") + buf, arr, 0);
    sampleManager->addSample(hh);
  }
  ...
  for (uint8_t i = 1; i < 8; i++) {
    ...
    DrumKit::Sample *hh =
        new DrumKit::Sample((uint64_t)size, std::string("cr78-") + buf, arr, 0);
    sampleManager->addSample(hh);
  }
  ...
  for (uint8_t i = 1; i < 13; i++) {
    ...
    DrumKit::Sample *hh =
        new DrumKit::Sample((uint64_t)size, std::string("dmx-") + buf, arr, 0);
    sampleManager->addSample(hh);
  }
  ...
  for (uint8_t i = 1; i < 15; i++) {
    ...
    DrumKit::Sample *hh =
        new DrumKit::Sample((uint64_t)size, std::string("tomi-") + buf, arr, 0);
    sampleManager->addSample(hh);
  }
}
